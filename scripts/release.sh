...
run_gradle_dependency_updates() {
  ...
  # Gradle sample versions are intentionally pinned and not auto-updated.
}
...
maven_update_phase() {
  ...
  # Maven sample is intentionally pinned; exclude it from version updates.
  ./mvnw -B -ntp org.codehaus.mojo:versions-maven-plugin:update-parent \
    -DallowSnapshots=false \
    -DgenerateBackupPoms=false \
    -Dmaven.version.ignore="$MAVEN_IGNORE_REGEX" \
    --projects '!samples/spring-boot-3.5-maven-sample'
